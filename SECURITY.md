...
- Backup contact: **[GitHub private security advisory](https://github.com/ICICLE-ai/intelligent-edge-management-service/security/advisories/new)**
...
Runtime configuration for this service is supplied through environment variables. Copy `edge_fleet_control_plane/.env.example` to `.env` and keep the populated `.env` out of version control. Never commit a populated `.env`, MQTT credentials, Tapis OAuth client keys, or application secrets.
