"""UNet++ TensorRT segmentation (pycuda, no PyTorch — Jetson Orin friendly).

Supports TensorRT 8+ IO tensors API (Orin / JP6). Falls back to TRT 7 bindings
when present (not expected on Orin Nano builds).
"""
...
class SharedUnetRunner:
    ...
    def load(self):
        if not os.path.exists(self.engine_path):
            print("CRITICAL: engine not found at %s" % self.engine_path)
            sys.exit(1)

        last_err = None
        for attempt in range(1, config.ENGINE_LOAD_RETRIES + 1):
            try:
                print("Loading UNet engine (attempt %d/%d): %s" % (
                    attempt, config.ENGINE_LOAD_RETRIES, self.engine_path,
                ))
        ...
        print(
            "HINT: GPU memory may be held by a previous container. On the host run:\n"
            "  docker ps -a && docker rm -f unetpp_infer_live\n"
            "  sudo tegrastats\n"
            "The .engine must be built on the same Jetson model (TRT cross-device warning)."
        )
